...
@section('content')
    ...
    <section class="Invoice">
        ...
        @endif
        <form action="/admin/productos/nuevo" method="post" id="FormProduct">
            {{ csrf_field() }}
                        ...
                        <div class="row between">
                            <label id="product_category_id" class="col-7 Label-select"><span>Categoria</span>
                                <select name="product_category_id" id="product_category_id">
                                    ...
                                    @foreach($categories as $key => $category )
                                        <option value="{{$key}}" {{(old('product_category_id') == $key)?'selected':''}}>
                                            {{$category}}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <label id="unity" class="col-7 Label-select"><span>Unidad</span>
                                <select name="unity" id="">
                                    ...
                                    <option value="1" {{((old('unity') == 1) ? 'selected':'')}}>Ninguno</option>
                                    <option value="2" {{((old('unity') == 2) ? 'selected':'')}}>Mes</option>
                                    <option value="3" {{((old('unity') == 3) ? 'selected':'')}}>Unidad</option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div class="row between">
                            <label for="net_price" class="col-16 "><span> Precio (sin IVA) </span>
                                <input type="text" id="net_price" name="net_price" class="alignRight"
                                       placeholder="Ingrese el precio $100.000" value="{{old('net_price')}}">
                            </label>
                        </div>
                        <div class="row between">
                            <label id="" class="col-7 Label-select"><span>IVA</span>
                                <select name="iva" id="iva">
                                    ...
                                    <option value="9" {{(old('iva') == 9)?'selected':''}}>9%</option>
                                    <option value="16" {{(old('iva') == 16)?'selected':''}}>16%</option>
                                    <option value="19" {{(old('iva') == 19)?'selected' :''}}>19%</option>
                                </select>
                            </label>
                            <label for="gross_price_with_iva" class="col-7 "><span> Precio bruto (con IVA) </span>
                                <input type="text" name="gross_price_with_iva" id="gross_price_with_iva" readonly
                                       class="alignRight" value="{{old('gross_price_with_iva')}}">
                            </label>
                        </div>
                        <div class="row between">
                            <label for="cost_price" class="col-16 "><span> Precio de coste </span>
                                <input type="text" name="cost_price" class="alignRight" id="cost_price"
                                       placeholder="Ingrese el precio de coste" value="{{old('cost_price')}}">
                            </label>
                        </div>
                    </div>
                </div>
